  ...
  <div class="app-container">
    <div class="data-operation">
      ...
      <div class="data-operation-item">
        <el-button type="primary" @click="redirectTo('/volume/pv-edit')">新增</el-button>
      </div>
    </div>
    <el-table :data="list">
      ...
      <el-table-column label="status" width="100" prop="status"></el-table-column>
      <el-table-column label="capacity" width="100" prop="capacity">
        <template slot-scope="{row}" style="cursor: pointer">
          {{ row.capacity }}Mi
        </template>
      </el-table-column>
      <el-table-column label="claim" width="100" prop="claim"></el-table-column>
      <el-table-column label="reClaimPolicy" width="120" prop="reClaimPolicy"></el-table-column>
      <el-table-column label="scName" width="100" prop="storageClassName"></el-table-column>
      <el-table-column label="reason" width="100" prop="reason"></el-table-column>
...
export default {
  ...
  methods: {
    getPVList() {
      ...
      this.$store.dispatch("pv/getPVList", params).then(res => {
        this.list = res.data
      })
    },
    filterPv() {
    ...
    doDel(name) {
      ...
      this.$confirm('此操作将永久删除该PersistentVolume, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$store.dispatch("pv/deletePV", params).then(res => {
