  ...
  <div class="app-container">
    <el-form ref="form" :model="form" :rules="rules" label-width="150px">
      ...
      <el-form-item label="type">
        <el-select style="width: 50%" v-model="form.type" placeholder="选择 service 类型">
      ...
      <el-form-item label="ports">
        <div v-for="(item,index) in form.ports" class="port-class">
          <el-row>
            <el-col class="col-class" :span="10" style="padding-right: 5px">
              <el-form-item style="margin-bottom: 5px" label="port">
                <el-input-number v-model="item.port"></el-input-number>
              </el-form-item>
              <el-form-item style="margin-bottom: 5px" label="targetPort">
                <el-input-number v-model="item.targetPort"></el-input-number>
              </el-form-item>
              <el-form-item v-if="form.type==='NodePort'" label="nodePort">
                <el-input-number v-model="item.nodePort"></el-input-number>
              </el-form-item>
              <el-button  @click="form.ports.splice(index,1)"
                         size="small"
                         type="danger">
                删除
                <i class="el-icon-delete"></i>
  ...
  methods: {
    ...
    load() {
      ...
      if (name !== undefined) {
        ...
        this.$store.dispatch("svc/getSvcItemOrList", params).then(res => {
          // this.form = res.data
          this.form = res.data
        })
      }
    ...
    onSubmit(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        this.$store.dispatch("svc/createSvc", this.form).then(res => {
