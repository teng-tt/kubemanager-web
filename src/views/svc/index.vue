  ...
  <div class="app-container">
    <div class="data-operation">
      ...
      <div class="data-operation-item">
        <el-button type="primary" @click="redirectTo('/discovery/service-edit')">新增</el-button>
      </div>
    </div>
    <el-table :data="list">
      ...
      <el-table-column label="clusterIP" width="150" prop="clusterIP" />
      <el-table-column label="externalIP" width="150" prop="externalIP" />
...
export default {
  ...
  methods: {
    doDel(name) {
      ...
      this.$confirm('此操作将永久删除该Service, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$store.dispatch("svc/deleteSvc", params).then(res => {
    ...
    goEdit(name) {
      this.redirectTo("/discovery/service-edit?name=" + name)
    },
    redirectTo(url) {
      this.$router.push(
        {
