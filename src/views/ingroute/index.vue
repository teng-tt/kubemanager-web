  ...
  <div class="app-container">
    <div class="data-operation">
      ...
      <div class="data-operation-item">
        <el-button type="primary" @click="redirectTo('/discovery/ingroute-edit')">新增</el-button>
      </div>
    </div>
    <el-table :data="list">
...
export default {
  ...
  methods: {
    doDel(name) {
      ...
      this.$confirm('此操作将永久删除该IngressRoute, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$store.dispatch("ingroute/deleteIngRoute", params).then(res => {
    ...
    goEdit(name) {
      this.redirectTo("/discovery/ingroute-edit?name=" + name)
    },
    redirectTo(url) {
      this.$router.push(
        {
